...
const fetchJournalEntries = async (token: string): Promise<JournalEntry[]> => {
  try {
    ...
    return entries.map((entry: any) => {
      ...
      let dateValue = entry.createdAt || entry.created_at
      let parsedDate: Date
      ...
      const imageUri = entry.imageUri || entry.image_uri || entry.image || entry.imageUrl || entry.image_url
      const audioUri = entry.audioUri || entry.audio_uri || entry.audio || entry.audioUrl || entry.audio_url
...
const createJournalEntry = async (entry: Omit<JournalEntry, 'id' | 'createdAt'>, token: string): Promise<JournalEntry> => {
  try {
    ...
    // Apply the same date parsing logic as fetchJournalEntries
    let dateValue = data.createdAt || data.created_at
    let parsedDate: Date
            ...
            entries.map((entry) => (
              <View key={entry.id} style={styles.entryCard}>
                ...
                <Text style={styles.entryDate}>
                  {entry.createdAt && !isNaN(entry.createdAt.getTime())
